...
void Quicksort_Hy(int *qarray , int first , int last)
{
...
if(first<last)
{
	...
	if(current_pivot - first > 20)
		Quicksort_Hy(qarray,first,current_pivot-1);
	else
		return;
		//Insertion_Sort_Iteration(qarray,first,current_pivot-1);

	if(last - current_pivot > 20)
		Quicksort_Hy(qarray,current_pivot+1,last);
	else
		return;
//		Insertion_Sort_Iteration(qarray,current_pivot+1,last);
}
else
	return;
...
void Quicksort_Hybrid(int *qarray , int first , int last)
{
Quicksort_Hy( qarray , first , last);
Insertion_Sort_Iteration(qarray,first,last);
}



void Merge(int *tarray,int tfirst,int t_q , int tlast)
{
int n1 = t_q-tfirst+1;
int n2 = tlast-t_q;
...
void Mergesort_Hybrid(int *marray,int first,int last)
{
...
if(first < last)
{
	...
	if(q - first+1 > 20)
		...
		Insertion_Sort_Iteration(marray,first,q);

	if(last - q > 20)
		Mergesort_Hybrid(marray,q+1,last);
	else
		Insertion_Sort_Iteration(marray,q+1,last);
...
void Variant_Linear_Search(int *a,int tsize,int key)
{
